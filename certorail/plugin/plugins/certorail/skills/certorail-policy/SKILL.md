...
1. **Load.** `certorail check -c "pass" --policy policy.toml --root ROOT`. Every schema
...
3. **Probes.** Write a small program in the subset per grant (the program-author guide,
   ...
   `certorail check probe.py --policy policy.toml --root ROOT`. The accepted report lists every
