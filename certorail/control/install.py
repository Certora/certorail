...
# ---------------------------------------------------------------------------
# the one verb: a file, a directory, or a pack shipped in this package
# ---------------------------------------------------------------------------


def shipped_packs() -> dict[str, Traversable]:
    """The ruleset packs shipped in this package, ``certorail/rulesets/<name>/``, by name."""
    base = importlib.resources.files("certorail").joinpath("rulesets")
    if not base.is_dir():
        return {}
    return {p.name: p for p in base.iterdir() if p.is_dir()}


def pack_installed(pack: Traversable) -> bool:
    """Is every ruleset of a shipped pack in the config directory, by name? Content is not
    compared: an installed pack the user has edited is theirs."""
    names = [p.name for p in pack.iterdir() if p.name.endswith(".toml")]
    return bool(names) and all((rulesets_dir() / n).is_file() for n in names)


def install(target: str, *, name: str | None = None, replace: bool = False) -> Report:
    """``certorail policy install TARGET``: a directory is a pack, a ``.toml`` file is a root
    policy, and a bare name is a pack shipped in this package."""
    path = pathlib.Path(target)
    if path.is_dir():
        if name is not None:
            raise InstallError(["--name names a policy's installed file; a pack has none"])
        return install_pack(path, replace=replace)
    if path.is_file() or path.suffix == ".toml":
        return install_policy(path, name=name, replace=replace)
    shipped = shipped_packs()
    if target in shipped:
        if name is not None:
            raise InstallError(["--name names a policy's installed file; a pack has none"])
        with importlib.resources.as_file(shipped[target]) as pack:
            return install_pack(pack, replace=replace)
    known = ", ".join(sorted(shipped)) or "none"
    raise InstallError([
        f"{target}: not a policy .toml, a pack directory, or a pack shipped with certorail (shipped: {known})"
    ])
...
def list_installed(root: pathlib.Path | None = None) -> str:
    ...
    them, when given)."""
    from certorail.control.apply import ruleset_lines  # the inventory lives with `apply`; imported here to avoid a cycle
    ...
    missing = [n for n, pack in sorted(shipped_packs().items()) if not pack_installed(pack)]
    if missing:
        out.append(
            "  shipped with certorail, not installed: "
            + ", ".join(f"{n} (certorail policy install {n})" for n in missing)
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    p_install = sub.add_parser(
        "install",
        help="install a root policy (a .toml file, for its declared root), a pack (a directory of "
        "ruleset .toml files plus checkers/), or a pack shipped with certorail (by name)",
    )
    p_install.add_argument("target", help="a policy .toml, a pack directory, or a shipped pack's name")
    p_install.add_argument("--name", default=None, help="a policy's installed file name (default: the source's)")
    p_install.add_argument("--replace", action="store_true",
    ...
    try:
        if ns.command == "install":
            report = install(ns.target, name=ns.name, replace=ns.replace)
            ...
            from certorail.control.apply import apply_ruleset, parse_binding
