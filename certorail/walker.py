"""Syntactic drivers: the validation walker and the violation analysis.

Kept apart from ``analysis`` (the domain and the expression semantics) so that this module can
import ``guards`` and ``annotations`` -- which themselves import ``analysis`` -- without a cycle:

    analysis  <-  terms, guards, annotations  <-  walker
"""
import ast
