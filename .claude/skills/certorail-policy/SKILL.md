...
| a tool that should see no more of the filesystem than the program may read (`cat`, `grep`, `ls`, `find`) | `exec.view = "policy"` on its rule: the child's world is the policy's `[filesystem]` section as mounts and nothing else (a path outside it does not exist). Only for tools that need nothing from the home directory; `git` and build tools keep the default `"host"`. On Linux, locations that are not a literal path or a literal `**` subtree have no mount and are reported at startup; macOS takes patterns as regexes |
