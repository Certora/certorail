...
`pack.toml` is the manifest and the source of truth: it names every artifact and every wiring
step. `install.py` knows three verbs -- `write_file`, `symlink`, `json_merge` -- and nothing about
...
Then it runs `certorail explain` with that invocation's own arguments -- the program or `-c`,
`--root`, `--policy` -- minus `--check` and `--no-jail`, and minus anything after a bare `--`,
which belongs to the program rather than to certorail. `explain` never runs the program.
...
from the repo root. It installs into a temporary HOME and touches nothing of the user's.
