...
def static(*parts: str | PseudoRegex) -> StaticPath:
    return StaticPath(tuple(Exact(p) if isinstance(p, str) else p for p in parts))


def splat(*prefix: str | PseudoRegex, final: PseudoRegex = ANY) -> DirSplat:
    return DirSplat(
        static_prefix=tuple(Exact(p) if isinstance(p, str) else p for p in prefix),
        final_component=final,
    )


def path_of(loc: LocationFact) -> ValidationFact:
    return ValidationFact(regex=None, containment=loc, atoms=frozenset(), type_info="path")


def validated(*atoms: AtomicFact, regex: PseudoRegex | None = None) -> ValidationFact:
    return ValidationFact(regex=regex, containment=None, atoms=frozenset(atoms), type_info="str")
...
class TestPathFromMultipleArgs(unittest.TestCase):
    ...
    def test_splat_fact_then_literal_fixes_final_component(self) -> None:
        self.assertEqual(
            ...
            path_of(splat("base", final=Exact("y"))),
        )
...
class TestJoinWithLiteral(unittest.TestCase):
    ...
    def test_splat_fixes_final_component(self) -> None:
        cases: list[tuple[str, DirSplat]] = [
            ('d / "x"', splat("base", final=Exact("x"))),
            ('d / "x/y"', splat("base", final=Exact("y"))),
        ]
        for src, loc in cases:
            with self.subTest(src=src):
...
class TestJoinWithPathFact(unittest.TestCase):
    ...
    def test_static_then_splat_extends_prefix(self) -> None:
        st: State = {"p": BASE, "q": path_of(splat("x", final=RegexLit(r"\w+")))}
        self.assertEqual(
            evaluate("p / q", st),
            path_of(splat("base", "x", final=RegexLit(r"\w+"))),
        )

    def test_splat_then_static_keeps_prefix_takes_final(self) -> None:
        st: State = {"d": UNDER_BASE, "q": path_of(static("x", "y"))}
        self.assertEqual(evaluate("d / q", st), path_of(splat("base", final=Exact("y"))))

    def test_splat_then_splat_keeps_prefix_takes_final(self) -> None:
        st: State = {"d": UNDER_BASE, "q": path_of(splat("x", final=RegexLit(r"\w+")))}
        self.assertEqual(evaluate("d / q", st), path_of(splat("base", final=RegexLit(r"\w+"))))
...
class TestJoinWithValidatedString(unittest.TestCase):
    ...
    def test_single_component_carries_its_regex(self) -> None:
        ...
        self.assertEqual(evaluate("p / s", st), path_of(static("base", r)))
    ...
    def test_splat_base_single_component_replaces_final(self) -> None:
        ...
        self.assertEqual(evaluate("d / s", st), path_of(splat("base", final=r)))
...
class TestJoinWithRegexDerivedAtoms(unittest.TestCase):
    ...
    def test_exact_filename_extends(self) -> None:
        ...
        self.assertEqual(evaluate("p / s", st), path_of(static("base", Exact("foo.txt"))))
    ...
    def test_alternation_of_safe_exacts_extends(self) -> None:
        ...
        self.assertEqual(evaluate("p / s", st), path_of(static("base", alt)))
    ...
    def test_concat_needs_atoms(self) -> None:
        ...
        self.assertEqual(evaluate("p / s", st), path_of(static("base", cat)))
...
class TestJoinOperandRestrictions(unittest.TestCase):
    ...
    def test_string_typed_left_operand_is_unknown(self) -> None:
        s = ValidationFact(regex=None, containment=static("base"), atoms=frozenset(), type_info="str")
        self.assertIsNone(evaluate('s / "x"', {"s": s}))

    def test_path_without_containment_is_unknown(self) -> None:
        p = ValidationFact(regex=None, containment=None, atoms=frozenset(), type_info="path")
        self.assertIsNone(evaluate('p / "x"', {"p": p}))
...
class TestCompoundExpressions(unittest.TestCase):
    ...
    def test_chained_join_onto_splat(self) -> None:
        self.assertEqual(
            ...
            path_of(splat("base", final=Exact("b"))),
        )
